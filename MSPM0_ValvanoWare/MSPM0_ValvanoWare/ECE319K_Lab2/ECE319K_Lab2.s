...
loop://Change switch on PB3
	 ...
	 LDR R1, =GPIOB_DOUT31_0
   	 LDR R0, [R1]
   	 LDR R3, =0b100000000000000000 //mask of bit 17
     EORS R0, R0, R3
     ...
     MOVS R0, R5
     BL Delay//delays for length duration

     PUSH {R2}
	 MOVS R3, #0
	 BL Switch_Input//checks the switch for input
	 POP {R2}
	 CMP R3, #8
	 BEQ step//if switch was pressed, incriment duty cycle
continue:
...
step:
     ADDS R2, R2, #4
     LDR R0, [R2]//incriments duty cycle
     CMP R0, #0
     BEQ reset//if at end of duty cycle, reset to beginning
...
Lab2Init:
   ...
   LDR R0, =0x0
   STR R0, [R2, #16]
   //Change to switch the duty cycle
   ADDS R2, R2, #4
   //
