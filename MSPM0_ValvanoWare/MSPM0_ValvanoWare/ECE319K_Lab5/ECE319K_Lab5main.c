...
const char EID1[] = "HPL294"; // replace abc123 with your EID
const char EID2[] = "NTG393"; // replace abc123 with your EID
//Key0=329.6, Key1=415.3, Key2=493.9, Key3=554.4 Hz
const uint32_t frequency[4] = { 330, 415, 494, 554 };
#define A5 880
#define G5 784
#define F5 698
...
int main5(void){// main5
  ...
  uint32_t last_in = 0;
  while(1){
      uint32_t input = Key_In();
      if (input != 0 && (last_in != input)) {
          if (input == 1) {
              Sound_Start(frequency[0]);
              Debug_Dump(frequency[0]);
          }
          if (input == 2) {
              Sound_Start(frequency[1]);
              Debug_Dump(frequency[1]);
          }
          if (input == 4) {
              Sound_Start(frequency[2]);
              Debug_Dump(frequency[2]);
          }
          if (input == 8) {
              Sound_Start(frequency[3]);
              Debug_Dump(frequency[3]);
          }

      }
      else if (input == 0 && last_in == input) {
          Sound_Stop();
      }
      last_in = input;
...
//Will run a song from an array of frequencies
...
int main7(void) {
    ...
    Sound_Start(250);//Begins playing song at 250Hz
    while(1){
...
void Sound_Init(uint32_t frequency, uint32_t priority){
    uint32_t period = 10000000000 / frequency;//converts to nanoseconds
    SysTick->CTRL = 0x00;
...
void Sound_Start(uint32_t frequency) {
  // set reload value
  // write any value to VAL, cause reload
    uint32_t period = 10000000000 / frequency;//converts to nanoseconds
    SysTick->LOAD = (period / 125)/32;//80000000*period/32;
    SysTick->VAL = 0;
}
...
uint32_t loc = 0;
const uint32_t Sound[32] = {
16,19,22,24,27,28,30,
31,31,31,30,28,27,24,
22,19,16,13,10,8,5,4,
2,1,1,1,2,4,5,8,10,13
};//2^7 to switch to 12 bit


void SysTick_Handler(void){
  // output one value to DAC
    uint32_t local_sound = Sound[loc];
    DAC5_Out(local_sound);
    loc = (loc+1)&0x1F;//Code for Sin output

    /*uint8_t local_sound = sandstorm[loc];
    DAC5_Out(local_sound);
    loc = (loc + 1);//Code for music
    if (loc > 26158) {
        loc = 0;
    }*/
