...
//    if zero, the signal is considered low.
  uint32_t periodsum = 0;//total of all lengths of periods
  uint32_t periodtotal = 0;//total number of periods
  uint32_t previousedge = 0;//index of previous rising edge
  uint32_t firstcomplete = 0;//verifies whether the first rising edge has been found
  if(DebugCnt <= 1){//Return when not enough data
      return 0;
  }
  

  for(uint8_t i = 1;i<DebugCnt;i++){
      if(((DataBuffer[i]&mask) > 0) && (DataBuffer[i-1]&mask) == 0){
          if(firstcomplete > 0){
              periodsum+= (TimeBuffer[previousedge]-TimeBuffer[i]);//Adds time difference between current rising edge and pervious one

              previousedge = i;//sets the last rising edge to the current index
              periodtotal++;
          }else{
              firstcomplete = 1;//if first rising edge doesnt do the calculation and just sets the edge
              previousedge = i;
          }
      }
  }
  if(periodtotal < 0){//ensures it doesnt / by 0
      return 0;
  }
...
//    if nonzero the signal is considered high.
//    if zero, the signal is considered low.
    uint32_t counter =0, difference=0, total=0, index=0, rising=0,falling=0,first_time = 0;
    while(index < 50){
        while((DataBuffer[index]&mask) == 0){
            index++;
            if(index >= 50 && counter > 0){
                return total/counter;
            }else if(index >=50 && counter ==0){
                return 0;
            }

        }
        if(index > 0 && first_time > 0){
            difference = rising-TimeBuffer[index];
            total += (100*falling)/difference;

            rising = TimeBuffer[index];
            counter++;
        }else if(index > 0 && first_time == 0){
            rising = TimeBuffer[index];
            first_time++;
        }
        ...
        if(first_time >0){
            falling = rising - TimeBuffer[index];
